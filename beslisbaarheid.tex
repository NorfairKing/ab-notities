...
\begin{de}
  $EQ_{CFG}$ is de taal van alle koppels CFG's $(C_{1},C_{2})$ die dezelfde taal beslissen.
...
\begin{st}
  ...
  \begin{proof}
    Een PDA kan in eindige tijd beslissen of een gegeven string uit gegeven grammatica afgeleid kan worden.
    Een co-herkenner $M$ voor $EQ_{CFG}$ verwacht als invoer twee CFG's $<C_{1},C_{2}>$ en gaat als volgt te werk:
    \begin{itemize}
    \item $M$ construeert voor beide CFG's de overeenkomstige PDA's
    \item $M$ laat beide PDA's simultaan lopen op alle strings van $Sigma^{*}$.
    \item Op het moment dat er een verschil wordt gevonden tussen de acceptatie van een string door $C_{1}$ en $C_{2}$ accepteert $M$ het koppel.
    \end{itemize}
  \end{proof}
\end{st}
...
\begin{st}
  \label{st:finite-csg-niet-herk}
  $FINITE_{CSG}$ is niet herkenbaar.
  
  \begin{proof}
    Met dezelfde reductie als in het vorige bewijs tonen we aan dat $FINITE_{CSG}$ niet herkenbaar is.\label{st:e-csg-niet-herk}
  \end{proof}
\end{st}

\begin{st}
  \label{st:finite-csg-niet-herk}
  $FINITE_{CSG}$ is co-herkenbaar.
  \extra{bewijs}
\end{st}
