...
\begin{de}
  Een \term{string} $s$ wordt \term{aanvaard door een NFA} $N=(Q,\Sigma,\delta,q_{s}F)$ als $s$ geschreven kan worden als een opeenvolging $a_{1}a_{2}\ldots a_{n}$ met $a_{i} \in \Sigma_{\epsilon}$ en er een opeenvolging toestanden $t_{1}t_{2}\ldots t_{n+1}$ bestaat zodat:
  \begin{itemize}
  \item $t_{1} = q_{s}$
...
\begin{st}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
    \item $|F| = 0 \vee |F| = 1$\\
      Wanneer $n$ hoogstens aanvaardbare eindtoestand bevat, is de NFA equivalent met een NFA met hoogstens \'e\'en aanvaardbare eindtoestand, namelijk zichzelf.\eiref{st:equivalentierelatie-NFA}.
    \item $|F| > 1$\\
...
\begin{de}
  ...
  \item $\sim_{DFA}$ verfijnt $\sim_{L}$:
    ...
    Of nog: ``De partitie bepaald door $\sim_{DFA}$ is fijner dan de partitie bepaald door $\sim_{L}$.''
